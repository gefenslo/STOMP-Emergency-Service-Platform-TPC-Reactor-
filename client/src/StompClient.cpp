...
#include <fstream>
#include <iostream>
#include "/workspaces/StompServer/client/include/event.h" 
#include <map>
#include <queue>
#include <mutex>
...
// Recursively search for a file by name starting from baseDir using POSIX dirent.h
std::string findFileInProject(const std::string& filename, const std::string& baseDir = "/workspaces/StompServer") {
    ...
    while ((entry = readdir(dir)) != nullptr) {
        ...
        if (entry->d_type == DT_DIR) {
            std::string found = findFileInProject(filename, fullPath);
            if (!found.empty()) {
                closedir(dir);
                return found;
            }
...
string getWordAtIndex(const string& str, size_t index) {
...
void communicationFunc() {
    ...
    if (command=="login"){
      ...
      string hostPort = getWordAtIndex(messageToSend,1);
        ...
        std::string file_name = getWordAtIndex(messageToSend,1);
        std::string foundPath = findFileInProject(file_name);
        if (foundPath.empty()) {
            std::cerr << "File not found: " << file_name << std::endl;
            // Do not return; just break out of this if/else so the loop continues
        } else {
            names_and_events events1 = parseEventsFile(foundPath);
        ...
        std::string outputFileName = getWordAtIndex(messageToSend,3);
        std::string outputDir = "/workspaces/StompServer/eventssummary";

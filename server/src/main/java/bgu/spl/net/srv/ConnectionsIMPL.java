...
ConcurrentHashMap<Integer,ConnectionHandler<String>> clientsMap;
Integer counter;
...
public ConnectionsIMPL(){
    clientsMap=new ConcurrentHashMap<Integer,ConnectionHandler<String>>();
    counter=0;
...
public boolean send(int connectionId, String msg){
    ConnectionHandler<String> handler=clientsMap.get(connectionId);
    if(handler==null){
        return false;
    }
    else{
...
public void disconnect(int connectionId){
    ...
    if(list!=null){
        ...
        LinkedList<String> topicsCopy = new LinkedList<>(list);
...
public ConcurrentHashMap<Integer, ConnectionHandler<String>> getClientsMap() {
    return clientsMap;
}
...
public boolean isClientSubscribedToChannel(int connectionId, String channel) {
    if (subscriptionsPerClient.containsKey(connectionId)) {
        LinkedList<String> topics = subscriptionsPerClient.get(connectionId);

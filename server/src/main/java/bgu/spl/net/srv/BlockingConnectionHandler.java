...
public class BlockingConnectionHandler<T> implements Runnable, ConnectionHandler<T> {
    ...
    public void run() {
        try (Socket sock = this.sock) { //just for automatic closing
            ...
            // Remove client from server on disconnect or error, with null checks
            try {
                ...
                try {
                    ...
                    // ignore, connectionId may not be set
                }
                if (connections != null && connectionId != null) {
                    connections.disconnect(connectionId);
                    connections.getClientsMap().remove(connectionId);
                    System.out.println("Cleaned up client with connectionId: " + connectionId);
                } else {
                    System.out.println("No cleanup needed: connections or connectionId is null");
                }
            } catch (Exception e) {
                // Suppressed error print during client cleanup
            }
        }
    }
